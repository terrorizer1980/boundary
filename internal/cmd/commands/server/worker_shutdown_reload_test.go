...
func TestServer_ShutdownReloadWorker(t *testing.T) {
	...
	sessionRepo, err := controllerCmd.controller.SessionRepoFn()
	require.NoError(err)
	...
	// Now, first off, let's just reload the server by sending a SIGHUP
	// "signal". We're not actually sending HUP, just sending one over
	// the worker's channel.
	logger.Debug("initiating reload")
	workerCmd.SighupCh <- struct{}{}

	// Wait for the reload.
	waitCh(t, workerCmd.reloadedCh)

	// The reload should have terminated the connection, to try to use
	// it, and expect failure.
	sConn.TestSendRecvFail(t)

	// Furthermore, on the DB side, the session should be marked as
	// closed.

	sess.ExpectConnectionStateOnController(ctx, t, sessionRepo, session.StatusClosed)

	// Now, let's test shutdown. Get a new session first.
	logger.Debug("re-connecting after reload")
	...
	// Connection should again fail, and the session should be closed
	// on the DB.
	sConn.TestSendRecvFail(t)
	sess.ExpectConnectionStateOnController(ctx, t, sessionRepo, session.StatusClosed)
